...
const ColorPaletteGenerator: React.FC = () => {
  ...
  const analyzeImage = useCallback(
    (imgElement: HTMLImageElement) => {
      ...
      const sortedBuckets = Object.entries(bucketMap).sort(
  ...
  const generatePalettes = (
    ...
    const limitedBuckets = bucketData.slice(
      0,
      Math.min(count, bucketData.length),
    );

    const createPalette = (
      hOffsetValue: number,
      isMullerAlg: boolean = false,
    ): ColorData[] => {
  ...
  return (
    <div
      className={`min-h-screen ${bgClass} p-8 transition-colors duration-300 w-full`}
    >
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h1
            className={`text-5xl font-bold ${darkMode ? "bg-gradient-to-r from-indigo-400 to-purple-400" : "bg-gradient-to-r from-indigo-600 to-purple-600"} bg-clip-text text-transparent mb-3`}
          >
            <MagicalText
